...
export class StatisticService {
  ...
  async faultCorrectionCapability(
    ...
    const T_bug = 1209600000; //604800000 (7 days) ;
    ...
    let bugs_corrected_in_time = 0;
    for (let k = 0; k < bug_closedAt.length; k++) {
      const start = new Date(bug_createdAt[k]).getTime();
      const end = new Date(bug_closedAt[k]).getTime();
      const difference = Math.abs(end - start);
      if (difference <= T_bug) {
        bugs_corrected_in_time += 1;
      }
    }
    const fault_correction_capability =
      Math.abs(bugs_corrected_in_time) / Math.abs(bugs);
    this.logger.log(
